...
const KpiCard = ({ title, value, subtext, icon: Icon, colorClass }: { title: string, value: string, subtext?: string, icon: any, colorClass: string }) => (
  <div className="bg-white border border-slate-200 rounded-2xl p-5 flex items-start justify-between relative overflow-hidden group hover:border-slate-300 shadow-sm transition-all">
    <div className={`absolute top-0 left-0 w-1 h-full ${colorClass}`} />
    <div>
      <p className="text-slate-500 text-sm font-medium mb-1">{title}</p>
      <h3 className="text-3xl font-display text-slate-900 font-bold">{value}</h3>
      {subtext && <p className="text-xs text-slate-400 mt-2">{subtext}</p>}
    </div>
    <div className={`p-3 rounded-xl bg-slate-100 ${colorClass.replace('bg-', 'text-')} opacity-80 group-hover:scale-110 transition-transform`}>
  ...
  return (
    <div className="mb-4 group">
      <div className="flex justify-between text-sm mb-1">
        <span className="font-bold text-slate-700 truncate max-w-[200px]">{name}</span>
        <div className="flex gap-4 text-xs font-mono">
          <span className="text-slate-500">{views} צפיות</span>
...
const BreakdownDonut = ({ items, title, icon: Icon, colorPalette }: {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
      <h3 className="text-lg text-slate-900 font-bold mb-5 flex items-center gap-2">
      ...
      <div className="flex items-center gap-6">
        ...
        >
          <div className="absolute inset-2 rounded-full bg-white flex items-center justify-center">
            <span className="text-slate-900 text-sm font-bold">{total}</span>
          </div>
        </div>
        {/* Legend */}
          ...
          {topItems.map((item, i) => (
            <div key={item.label} className="flex items-center justify-between text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-slate-400 text-xs w-12 text-left">{item.percent}%</span>
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
      ...
      <div className="space-y-3">
        {topItems.map((item) => (
          <div key={item.label}>
            <div className="flex justify-between text-sm mb-1">
              <span className="text-slate-600">{labelMap[item.label] || item.label}</span>
              <span className="text-slate-400 font-mono text-xs">{item.count} ({item.percent}%)</span>
            </div>
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl text-slate-900 font-display">ביצועי המערכת</h2>
          <p className="text-slate-500 text-sm">סקירה בזמן אמת של התעניינות ומכירות</p>
        </div>
        <button
          onClick={fetchSummary}
          className="px-4 py-2 bg-slate-50 hover:bg-slate-100 text-slate-700 text-sm rounded-lg border border-slate-200 transition flex items-center gap-2"
        >
      ...
      {activeTab === 'overview' && (
        <>
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex bg-slate-100 rounded-lg p-1">
                <button
                  ...
                  className={`px-3 py-1 text-sm rounded-md transition-all ${timeFilter === 'daily' ? 'bg-white text-slate-900 font-medium shadow-sm' : 'text-slate-500 hover:text-slate-900'} disabled:opacity-50`}
                >
                  7 ימים (לפי שעה)
                </button>
                <button
                  ...
                  disabled={isLoadingChart}
                  className={`px-3 py-1 text-sm rounded-md transition-all ${timeFilter === 'hourly' ? 'bg-white text-slate-900 font-medium shadow-sm' : 'text-slate-500 hover:text-slate-900'} disabled:opacity-50`}
                >
                  שעתי (24 שעות)
                </button>
              </div>
                  ...
                  {chartData.map((d, i) => {
                    ...
                    return (
                      <div key={i} className="relative flex-1 group flex flex-col justify-end items-center cursor-pointer">
                        <div className="absolute top-0 bottom-0 w-[1px] bg-slate-300 opacity-0 group-hover:opacity-100 transition-opacity" />
                        <div
                          className="w-2 h-2 rounded-full bg-jungle-accent opacity-0 group-hover:opacity-100 absolute transition-opacity shadow-[0_0_8px_rgba(37,99,235,0.5)]"
                          style={{ bottom: `${heightPercent}%` }}
                        />
                        <div dir="rtl" className="absolute bottom-full mb-2 bg-slate-800 backdrop-blur border border-slate-700 text-white text-xs p-3 rounded-lg shadow-xl opacity-0 group-hover:opacity-100 transition-all pointer-events-none transform translate-y-2 group-hover:translate-y-0 whitespace-nowrap z-20 min-w-[120px]">
                          <p className="font-bold text-sm mb-1 border-b border-slate-600 pb-1 text-center">{d.label}</p>
                        ...
                        {(!detailedData || detailedData.range !== '7d' || i % 12 === 0) && (
                          <div className="absolute -bottom-6 left-0 right-0 flex justify-center">
                            <span className="text-[10px] text-slate-400 rotate-0 truncate px-1 whitespace-nowrap">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
              <h3 className="text-xl text-slate-900 font-bold mb-6">טעוני שיפור ⚠️</h3>
              <p className="text-xs text-slate-400 mb-4">מסיבות עם חשיפה גבוהה אך מעט הקלקות (Low CTR)</p>
              <div className="space-y-4">
                  ...
                  .map(party => (
                    <div key={party.partyId} className="flex items-center justify-between p-3 bg-red-500/5 rounded-lg border border-red-500/10">
                      <div className="overflow-hidden">
                        <p className="text-sm font-bold text-slate-700 truncate">{party.name}</p>
                ...
                {stats.sortedByViews.filter(p => p.views > 10 && calculateCTR(p.views, p.redirects) < 2).length === 0 && (
                  <p className="text-slate-400 text-sm text-center py-4">אין כרגע מסיבות עם ביצועים נמוכים חריגים 👏</p>
                )}
              </div>
            </div>
          </div>
                ...
                <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 text-xs bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-md border border-slate-200 transition whitespace-nowrap"
                    >
                      📥 ייצוא CSV
                    </button>
                  </div>
                      ...
                      <tbody>
                        {visitorData.visitors.slice(visitorPage * VISITOR_PAGE_SIZE, (visitorPage + 1) * VISITOR_PAGE_SIZE).map((v, i) => (
                          <tr key={i} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                            <td className="py-2 px-3 text-slate-600 font-mono text-xs whitespace-nowrap">
                            ...
                            <td className="py-2 px-3">
                              <span className={`px-2 py-0.5 rounded-full text-xs ${v.deviceType === 'mobile' ? 'bg-green-500/10 text-green-600' :
                                v.deviceType === 'desktop' ? 'bg-blue-500/10 text-blue-600' :
                                  'bg-gray-500/10 text-slate-500'
                                }`}>
                                {v.deviceType === 'mobile' ? '📱' : v.deviceType === 'desktop' ? '🖥️' : '📟'} {v.deviceType}
                              </span>
                            </td>
                            <td className="py-2 px-3 text-slate-500">{v.browser}</td>
                            <td className="py-2 px-3 text-slate-500">{v.os}</td>
                            <td className="py-2 px-3">
                              <span className={`px-2 py-0.5 rounded-full text-xs ${v.trafficSource === 'social' ? 'bg-purple-500/10 text-purple-600' :
                                v.trafficSource === 'organic_search' ? 'bg-cyan-500/10 text-cyan-600' :
                                  v.trafficSource === 'direct' ? 'bg-gray-500/10 text-slate-500' :
                            ...
                            <td className="py-2 px-3 text-slate-400 text-xs truncate max-w-[200px]" title={v.referer}>
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="px-3 py-1 text-xs bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-md border border-slate-200 transition disabled:opacity-30 disabled:cursor-not-allowed"
                        >
                          הקודם
                        </button>
                        <button
      ...
      {activeTab === 'parties' && (
        <>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
            <h3 className="text-xl text-slate-900 font-bold mb-6 flex items-center gap-2">
            ...
            <div className="space-y-6">
              ...
              {stats.sortedByViews.length === 0 && (
                <p className="text-slate-400 text-center py-8">אין אירועים פעילים כרגע</p>
              )}
            </div>
          </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="bg-white border border-slate-300 text-slate-700 text-sm rounded-md px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-jungle-accent w-full sm:w-56"
                />
                <button
                  onClick={() => exportPartiesToCsv(filteredPartyTableRows)}
                  disabled={filteredPartyTableRows.length === 0}
                  className="px-3 py-1.5 text-xs bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-md border border-slate-200 transition disabled:opacity-30 disabled:cursor-not-allowed whitespace-nowrap"
                >
                  📥 ייצוא CSV
                </button>
              </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {partyTablePageRows.map(party => {
                    ...
                    return (
                      <tr key={party.partyId} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                        <td className="py-2 px-3 text-slate-700 font-medium truncate max-w-[200px]">{party.name}</td>
                        <td className="py-2 px-3 text-slate-400 font-mono text-xs">{party.slug}</td>
                        <td className="py-2 px-3 text-slate-500 text-xs whitespace-nowrap">
                          {party.date ? new Date(party.date).toLocaleDateString('he-IL') : '-'}
                        </td>
                        <td className="py-2 px-3 text-blue-600 font-mono">{party.views}</td>
                        <td className="py-2 px-3 text-jungle-lime font-mono">{party.redirects}</td>
                        <td className="py-2 px-3">
                          <span className={`font-mono font-bold ${ctr > 5 ? 'text-green-600' : ctr > 2 ? 'text-yellow-600' : 'text-red-600'}`}>
                            {ctr.toFixed(1)}%
                          </span>
                        </td>
                        ...
                        <td className="py-2 px-3">
                          {realConversion !== null ? (
                            <span className={`font-mono font-bold ${realConversion > 5 ? 'text-green-600' : realConversion > 2 ? 'text-yellow-600' : 'text-red-600'}`}>
                              {realConversion.toFixed(1)}%
                            </span>
                          ) : (
                            <span className="text-slate-500 text-xs">אין נתון</span>
                          )}
                        </td>
                      </tr>
                    );
                  ...
                  {partyTablePageRows.length === 0 && (
                    <tr>
                      <td colSpan={8} className="py-6 text-center text-slate-400">לא נמצאו אירועים תואמים</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Best CTR */}
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
              <h3 className="text-lg text-slate-900 font-bold mb-4">🏆 אלופי ההמרות</h3>
              <p className="text-xs text-slate-400 mb-4">אירועים עם יחס המרה הגבוה ביותר (מינימום 5 צפיות)</p>
              <div className="space-y-3">
                ...
                {stats.sortedByCTR.length === 0 && (
                  <p className="text-slate-400 text-sm text-center py-4">אין מספיק נתונים</p>
                )}
              </div>
            </div>
            ...
            {/* Low CTR */}
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
              <h3 className="text-lg text-slate-900 font-bold mb-4">⚠️ טעוני שיפור</h3>
              <p className="text-xs text-slate-400 mb-4">אירועים עם חשיפה גבוהה אך יחס המרה נמוך</p>
              <div className="space-y-3">
                {stats.sortedByViews
                  .filter(p => p.views > 10 && calculateCTR(p.views, p.redirects) < 2)
                  .slice(0, 5)
                ...
                {stats.sortedByViews.filter(p => p.views > 10 && calculateCTR(p.views, p.redirects) < 2).length === 0 && (
                  <p className="text-slate-400 text-sm text-center py-4">אין כרגע אירועים עם ביצועים נמוכים חריגים 👏</p>
                )}
              </div>
            </div>
          </div>
